...
PiTree is a multidimensional in-memory learned tree index structure.
The data is stored as one in-memory array.
Each node is responsible for indexing a range of the data array in $[start, end)$.
Nodes contain a projection vector $proj$ of the same dimension of the data
which it uses to sort the range of data it indexes. 
To be precise, if $d_1, d_2$ are two datapoints, the node will 
sort $d_1$ before $d_2$ if $d_1 \cdot proj < d_2 \cdot proj$.
